...
— loading the actual bundled TTF via Compose Multiplatform's resource system instead of delegating to the per-backend `Monospace` alias. This compiles cleanly against all three targets (`:shared:compileAndroidMain`, `:shared:compileKotlinJvm`, `:shared:compileKotlinIosSimulatorArm64`). **As of this writing, that is the only verification performed** — the fix has not yet been re-run on an actual Android emulator/device to visually confirm the columns now line up, which per this document's own standard (§8's insistence on real execution over compilation) means it should be treated as *applied, not yet confirmed*, until that check happens. A secondary, untested hypothesis worth naming: `mesloMonoTextStyle.kt` also sets `letterSpacing = 0.2.sp`, and fractional letter-spacing has historically had different rounding/hinting behavior between Android's native renderer and Skia; if misalignment persists after the font fix, this is the next place to look.
